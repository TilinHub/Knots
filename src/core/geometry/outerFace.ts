...
function convexHullContour(disks: ContactDisk[]): EnvelopeSegment[] {
    ...
    for (let i = 0; i < n; i++) {
        ...
        const departAngle = Math.atan2(currTangent.from.y - d.center.y, currTangent.from.x - d.center.x);
...
function jarvisMarchIndices(disks: ContactDisk[]): number[] {
    ...
    for (let i = 1; i < disks.length; i++) {
        if (disks[i].center.y < disks[startIdx].center.y ||
            (disks[i].center.y === disks[startIdx].center.y && disks[i].center.x < disks[startIdx].center.x)) {
            startIdx = i;
        }
    }
    ...
    do {
        ...
        for (let i = 0; i < disks.length; i++) {
            ...
            const isFurther = dist(disks[current].center, disks[i].center) >
