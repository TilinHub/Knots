...
export function reconstructGeometry(
    ...
    for (let i = 0; i < resolved.length; i++) {
        ...
        if (prevSeg && prevSeg.type !== 'ARC') {
            ...
            arcSeg.startAngle = Math.atan2(
                endPt.y - disk.center.y,
                endPt.x - disk.center.x
            );
        }
        ...
        if (nextSeg && nextSeg.type !== 'ARC') {
            ...
            arcSeg.endAngle = Math.atan2(
                startPt.y - disk.center.y,
                startPt.x - disk.center.x
            );
        }
...
function computeBitangent(
    ...
    if (tangentType === 'LSL' || tangentType === 'RSR') {
        ...
        if (D < d1.radius + d2.radius - EPSILON) return null;

        const val = (d1.radius + d2.radius) / D;
        const beta = Math.acos(Math.max(-1, Math.min(1, val)));
...
function segmentPenetratesDisk(p1: Point2D, p2: Point2D, disk: DiskGeometry): boolean {
    ...
    const t = Math.max(0, Math.min(1,
        ((disk.center.x - p1.x) * dx + (disk.center.y - p1.y) * dy) / len2
    ));
