 ...
 */
export interface AngularRangeConfig {
  // Renamed to avoid partial overlap if any, but actually AngularRange is strict
...
export function solveAngularDubins(
  ...
  const validCandidates = obstacles
    ? candidates.filter((p) => !checkDubinsPathCollision(p, obstacles))
    : candidates;
