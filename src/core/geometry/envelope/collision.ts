import type { Config, DubinsPath, Obstacle, Point } from '../dubins/dubins';
...
export function checkDubinsPathCollision(path: DubinsPath, obstacles: Obstacle[]): boolean {
  ...
  if (len1 > EPSILON) {
    ...
    if (type === 'S') {
      ...
      for (const obs of obstacles) {
        if (
          checkSegmentCircleCollision(
            { x: currentConfig.x, y: currentConfig.y },
            { x: nextX, y: nextY },
            obs,
          )
        )
      ...
      for (const obs of obstacles) {
        if (checkArcCircleCollision({ x: cx, y: cy }, r1, startAngle, endAngle, isCCW, obs))
          return true;
      }
  ...
  if (len2 > EPSILON) {
    ...
    if (type === 'S') {
      ...
      for (const obs of obstacles) {
        if (checkArcCircleCollision({ x: cx, y: cy }, r2, startAngle, endAngle, isCCW, obs))
          return true;
      }
  ...
  if (len3 > EPSILON) {
    ...
    if (type === 'S') {
      ...
      for (const obs of obstacles) {
        if (checkArcCircleCollision({ x: cx, y: cy }, r3, startAngle, endAngle, isCCW, obs))
          return true;
      }
    }
  }
