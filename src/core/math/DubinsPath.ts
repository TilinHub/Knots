...
export class DubinsPathCalculator {
  ...
  public computeOptimalPath(start: Pose2D, end: Pose2D): DubinsPath | null {
    ...
    // Filter valid paths
    const validPaths = paths.filter((p): p is DubinsPath => 
      p !== null && p.isValid
    );
  ...
  public computeLRL(start: Pose2D, end: Pose2D): DubinsPath | null {
    ...
    const segments: DubinsSegment[] = [
      ...
      },
      {
        ...
        endPose: { position: tangent2, theta: this.getTangentAngle(middleCircle, tangent2, 'right') },
        center: middleCircle,
        radius: this.minRadius
      },
      {
        type: 'L',
        length: arc3Length,
        startPose: { position: tangent2, theta: this.getTangentAngle(middleCircle, tangent2, 'right') },
        endPose: end,
        center: clEnd,
        radius: this.minRadius
      }
  ...
  public computeRLR(start: Pose2D, end: Pose2D): DubinsPath | null {
    ...
    const segments: DubinsSegment[] = [
      ...
      },
      {
        ...
        endPose: { position: tangent2, theta: this.getTangentAngle(middleCircle, tangent2, 'left') },
        center: middleCircle,
        radius: this.minRadius
      },
      {
        type: 'R',
        length: arc3Length,
        startPose: { position: tangent2, theta: this.getTangentAngle(middleCircle, tangent2, 'left') },
        endPose: end,
        center: crEnd,
        radius: this.minRadius
      }
