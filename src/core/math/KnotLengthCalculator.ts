...
export class KnotLengthCalculator {
  ...
  public computeDetailedAnalysis(embedding: KnotEmbedding): KnotLengthAnalysis {
    ...
    const curvatures = dubinsPaths
      ...
      .flatMap(p => p.segments.map(s => 
        s.type !== 'S' ? (1 / (s.radius || this.config.minRadius)) : 0
      ));
    
    const maxCurvature = Math.max(...curvatures, 0);
    const averageCurvature = curvatures.length > 0 
  ...
  public compareEmbeddings(
    embedding1: KnotEmbedding,
    embedding2: KnotEmbedding
  ): ComparisonResult {
    ...
    const curvatureDiff = Math.abs(
      analysis1.averageCurvature - analysis2.averageCurvature
    );
