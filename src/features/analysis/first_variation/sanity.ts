...
function runSanityCheck() {
  ...
  if (tangentRes.results.some((r) => !r.passed)) {
    console.error(
      'Tangent checks failed:',
      tangentRes.results.filter((r) => !r.passed),
    );
    return;
  }
  ...
  if (segRes.some((r) => !r.passed)) {
    console.error(
      'Segment checks failed:',
      segRes.filter((r) => !r.passed),
    );
    return;
  }
