...
import {
  checkAndComputeTangents,
  checkArcs,
  checkCombinatorial,
  checkGlobalIntersections,
  checkImmediateMetrics,
  checkSegments,
} from './checks';
...
export function analyzeDiagram(diagram: CSDiagram): AnalysisReport {
  try {
    ...
    const allMetrics = [...metrics, ...tangentRes.results, ...segmentChecks, ...arcChecks];
    ...
    const pdfReport =
      combinatorial.passed && A && Tc && Tw && L && gc && gw && crit
        ? {
            ...
            E: diagram.contacts.map((c) => {
              ...
              return {
                ...
                j: c.diskB,
                dist: d,
                valid: Math.abs(d - 2) < diagram.tolerances.met,
              };
            }),
