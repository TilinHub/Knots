...
const MathVar = ({ children }: { children: React.ReactNode }) => (
  <span style={{ fontFamily: TEX_FONT, fontStyle: 'italic' }}>{children}</span>
);

// Numbers/Operators usually roman (upright)
const MathNum = ({ children }: { children: React.ReactNode }) => (
  <span style={{ fontFamily: TEX_FONT, fontStyle: 'normal' }}>{children}</span>
);
...
const ResultRow = ({
  ...
  >
    ...
    <div style={{ textAlign: 'right' }}>
      <span
        style={{
          color: passed ? 'var(--accent-success)' : 'var(--accent-error)',
          fontWeight: 'bold',
        }}
      >
        {value}
...
const DataRow = ({
  ...
  <div
    style={{ display: 'flex', justifyContent: 'space-between', padding: '2px 0', fontSize: '13px' }}
  >
    <span style={{ color: 'var(--text-secondary)' }}>{label}</span>
    ...
    <div style={{ borderBottom: '1px solid var(--border)', padding: '4px 0' }}>
      ...
      >
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          >
            {passed ? 'PASS' : `FAIL (${failedItems.length})`}{' '}
        ...
        >
          {failedItems.map((m, i) => (
            <div key={i} style={{ marginBottom: '4px', color: 'var(--accent-error)' }}>
              • {m.message}{' '}
              {m.value > 0 && (
                <span style={{ opacity: 0.7 }}>(val: {m.value.toExponential(2)})</span>
              )}
            </div>
          ))}
        </div>
...
const AnalysisChecklist = ({
  ...
  report: NonNullable<AnalysisResultsPanelProps['pdfReport']>;
}) => {
  return (
    <div style={{ padding: '0 4px', fontSize: '13px' }}>
      ...
      <ChecklistSection
        title={
          <span>
            2. Contactos (<MathText>ℰ</MathText>)
          </span>
        }
      >
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          <thead>
            <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--border)' }}>
              <th style={{ padding: '4px' }}>
                Pair <MathNum>{`{i, j}`}</MathNum>
              </th>
              <th style={{ padding: '4px' }}>
                Dist check <MathNum>||</MathNum>
                <MathText>
                  c<sub>i</sub>
                </MathText>
                <MathNum> - </MathNum>
            ...
            {report.E.map((c, idx) => (
              <tr key={idx} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                <td style={{ padding: '4px' }}>
                  <MathText>{`{${c.i}, ${c.j}}`}</MathText>
                </td>
                <td
                  style={{
                    padding: '4px',
                    fontFamily: 'monospace',
                    color: c.valid ? 'var(--accent-success)' : 'var(--accent-error)',
                  }}
                >
      ...
      <ChecklistSection
        title={
          <span>
            3. Tangencias (<MathText>𝒯</MathText>)
          </span>
        }
      >
            ...
            <tbody>
              {report.T.map((t) => (
                <tr key={t.id} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                  ...
                  <td style={{ padding: '4px', fontFamily: 'monospace' }}>
                    ({t.p.x.toFixed(3)}, {t.p.y.toFixed(3)})
                  </td>
                  <td style={{ padding: '4px', fontFamily: 'monospace' }}>
                    ({t.n.x.toFixed(3)}, {t.n.y.toFixed(3)})
                  </td>
                  <td style={{ padding: '4px', fontFamily: 'monospace' }}>
                    ({t.t.x.toFixed(3)}, {t.t.y.toFixed(3)})
                  </td>
                  <td style={{ padding: '4px' }}>{t.epsilon > 0 ? '+1' : '-1'}</td>
                  <td
                    style={{
                      padding: '4px',
                      color: t.validDist ? 'var(--accent-success)' : 'var(--accent-error)',
                    }}
                  >
                    {t.validDist ? '✓' : '✗'}
                  </td>
      ...
      <ChecklistSection
        title={
          <span>
            4. Matrices (<MathText>L</MathText>
            <MathNum>(c)</MathNum>)
          </span>
        }
      >
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <DataRow
            ...
            value={report.matrices.A_dims}
            sub={`Rank ${report.matrices.A_rank}`}
          />
          <DataRow
            label={
              <span>
            ...
            value={report.matrices.Tc_dims}
            sub={`Rank ${report.matrices.Tc_rank}`}
          />
          <DataRow
            label={
              <span>
            ...
            value={report.matrices.Tw_dims}
            sub={`Rank ${report.matrices.Tw_rank}`}
          />
          <DataRow
            label={
              <span>
            ...
            value={report.matrices.L_dims}
            sub={`Rank ${report.matrices.L_rank}`}
          />
        </div>
      </ChecklistSection>
      ...
      <ChecklistSection
        title={
          <span>
            5. Combinatoria (<MathText>𝒮</MathText>
            <MathNum>, </MathNum>
            <MathText>𝒜</MathText>)
          </span>
        }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          <div>
            <strong style={{ display: 'block', marginBottom: '4px' }}>
              Segmentos (<MathText>α</MathText> <MathNum>→</MathNum> <MathText>β</MathText>)
            </strong>
            {report.S.map((s, i) => (
              <div key={i}>
                <MathText>{s.alpha}</MathText> <MathNum>→</MathNum> <MathText>{s.beta}</MathText>
              </div>
            ))}
          </div>
          <div>
            <strong style={{ display: 'block', marginBottom: '4px' }}>
              Arcos (<MathText>α</MathText> <MathNum>→</MathNum> <MathText>β</MathText>
              <MathNum>, </MathNum>
              <MathText>k</MathText>
              <MathNum>, </MathNum>
              <MathText>σ</MathText>)
            ...
            {report.A.map((a, i) => (
              <div key={i}>
                <MathText>{a.alpha}</MathText> <MathNum>→</MathNum> <MathText>{a.beta}</MathText>
                <MathNum>
                  , k={a.k}, σ={a.sigma > 0 ? '+' : '-'}
                </MathNum>
              </div>
      ...
      <ChecklistSection
        title={
          <span>
            6. Funcional (<MathText>Φ</MathText>)
          </span>
        }
      >
      ...
      <ChecklistSection title={<span>7. Criticidad y Segunda Variación</span>}>
        <div
          style={{
            background: report.Criticality.isCritical ? 'rgba(0,255,0,0.05)' : 'rgba(255,0,0,0.05)',
            padding: '12px',
            borderRadius: '8px',
          }}
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            ...
            <strong
              style={{
                color: report.Criticality.isCritical
                  ? 'var(--accent-success)'
                  : 'var(--accent-error)',
              }}
            >
          ...
          <DataRow
            label={<MathText>||r||</MathText>}
            value={report.Criticality.r_norm.toExponential(6)}
          />
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>
                ...
                <span style={{ fontSize: '10px' }}>(Segunda Variación)</span>
              </span>
              <span style={{ fontFamily: 'monospace', fontWeight: 'bold' }}>
                {report.Criticality.Q_red !== undefined
                  ? report.Criticality.Q_red.toExponential(4)
                  : 'N/A'}
              </span>
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button onClick={onClose} variant="secondary" style={{ padding: '4px 8px' }}>
          ✕
        </Button>
      </div>
        ...
        <>
          ...
          {instanceData && (
            ...
            >
              ...
              >
                <span>
                  Table with <MathText>N</MathText>
                  <MathNum>, </MathNum>
                  <MathText>
                    c<sub>i</sub>
                  </MathText>
              ...
              {instanceExpanded && (
                <div
                  style={{ padding: '12px', background: 'var(--bg-secondary)', fontSize: '12px' }}
                >
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <ResultRow
              label={
                <span>
                  <MathNum>(</MathNum>
                  <MathText>C</MathText>
                  <MathNum>0)</MathNum> verificacion del ciclo orientado
                </span>
              }
            ...
            <DetailedResultRow
              label={
                <span>
                  Verificaciones metricas <MathNum>(</MathNum>
                  <MathText>S</MathText>
                  <MathNum>1)-(</MathNum>
                  <MathText>S</MathText>
                  <MathNum>2), (</MathNum>
            ...
            <DetailedResultRow
              label={
                <span>
                  Chequeos globales <MathNum>(</MathNum>
                  <MathText>G</MathText>
                  <MathNum>1)-(</MathNum>
                  <MathText>G</MathText>
                  <MathNum>3)</MathNum>
              ...
              <div>
                ...
                >
                  <DataRow
                    ...
                    value={matrices.A.dims}
                    sub={`Rank ${matrices.A.rank}`}
                  />
                  <DataRow
                    label={
                      <span>
                    ...
                    value={matrices.Tc.dims}
                    sub={`Rank ${matrices.Tc.rank}`}
                  />
                  <DataRow
                    label={
                      <span>
                    ...
                    value={matrices.Tw.dims}
                    sub={`Rank ${matrices.Tw.rank}`}
                  />
                  <DataRow
                    label={
                      <span>
                    ...
                    value={matrices.L.dims}
                    sub={`Rank ${matrices.L.rank}`}
                  />
                </div>
              </div>
              <div>
                ...
                >
                  <DataRow
                    ...
                    value={`||·|| = ${vectors.gc.norm.toExponential(4)}`}
                    sub={vectors.gc.dims}
                  />
                  <DataRow
                    label={
                    ...
                    value={`||·|| = ${vectors.gw.norm.toExponential(4)}`}
                    sub={vectors.gw.dims}
                  />
                  ...
                  <DataRow
                    ...
                    value={`||·|| = ${vectors.gred.norm.toExponential(4)}`}
                    sub={vectors.gred.dims}
                  />
                </div>
              </div>
            </div>
                ...
                <div>
                  ...
                  <DataRow
                    label={
                      <span>
                        <MathNum>||</MathNum>
                        <MathText>A U</MathText>
                        <MathNum>||</MathNum>
                      </span>
                    }
                    value={gauge.checks.AUc.toExponential(2)}
                  />
                  <DataRow
                    label={
                      <span>
                        <MathNum>||</MathNum>
                        <MathText>
                          U<sup>T</sup>U <MathNum>-</MathNum> I
                        </MathText>
                        <MathNum>||</MathNum>
                      </span>
                    }
                    value={gauge.checks.UtU_I.toExponential(2)}
                  />
                  <DataRow
                    label={
                      <span>
                        <MathNum>||</MathNum>
                        <MathText>
                          W<sup>T</sup>W <MathNum>-</MathNum> I
                        </MathText>
                        <MathNum>||</MathNum>
                      </span>
                    }
                    value={gauge.checks.WtW_I.toExponential(2)}
                  />
                  <DataRow
                    label={
                      <span>
                    ...
                    value={gauge.checks.UgtW.toExponential(2)}
                  />
                </div>
              </div>
            </div>
          ...
          {criticality && (
            <div
              style={{
                background: criticality.isCritical ? 'rgba(0,255,0,0.05)' : 'rgba(255,0,0,0.05)',
                padding: '16px',
                borderRadius: '8px',
                marginTop: '20px',
              }}
            >
              ...
              >
                <h4 style={{ margin: 0, fontSize: '14px' }}>
                  Criticidad: <MathText>r</MathText> <MathNum>=</MathNum> <MathText>U</MathText>
                  <MathNum>
                    <sub>g</sub>
                    <sup>T</sup>
                  </MathNum>{' '}
                ...
                <span
                  style={{
                    fontWeight: 'bold',
                    color: criticality.isCritical ? 'var(--accent-success)' : 'var(--accent-error)',
                  }}
                >
              ...
              <div style={{ fontFamily: 'monospace', fontSize: '13px' }}>
                <div>
                  <MathNum>||</MathNum>
                  <MathText>r</MathText>
                  <MathNum>||</MathNum> &nbsp;&nbsp; {criticality.normR.toExponential(6)}
                </div>
              ...
              {quadratic !== undefined && (
                ...
                >
                  ...
                  <div style={{ fontFamily: 'monospace' }}>
                    ...
                    <MathText>r</MathText>
                    <MathNum>)</MathNum>: {quadratic.toExponential(4)}
                  </div>
                </div>
              )}
            </div>
