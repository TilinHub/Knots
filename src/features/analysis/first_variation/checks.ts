...
export function checkCombinatorial(diagram: CSDiagram): CheckResult {
  ...
  for (const p of allPieces) {
    if (adj.has(p.startTangencyId)) {
      return {
        passed: false,
        value: 1,
        message: `Tangency ${p.startTangencyId} has multiple outgoing pieces`,
      };
    }
  ...
  if (outgoing.size !== tangencies.length || incoming.size !== tangencies.length) {
    return {
      passed: false,
      value: Math.abs(outgoing.size - tangencies.length),
      message: 'Mismatch in number of pieces and tangencies',
    };
  }
  ...
  const isSingleCycle =
    count === tangencies.length &&
    curr === (segments[0]?.startTangencyId || arcs[0]?.startTangencyId);
...
export function checkGlobalIntersections(diagram: CSDiagram): CheckResult[] {
  ...
  for (let i = 0; i < segments.length; i++) {
    for (let j = i + 1; j < segments.length; j++) {
      ...
      const areNeighbors =
        Math.abs(i - j) === 1 || (i === 0 && j === segments.length - 1 && segments.length > 2);
  ...
  for (let i = 0; i < segments.length; i++) {
    for (let j = 0; j < arcs.length; j++) {
      ...
      // Skip adjacent
      if (
        s.endTangencyId === a.startTangencyId ||
        s.startTangencyId === a.endTangencyId ||
        s.endTangencyId === a.endTangencyId ||
        s.startTangencyId === a.startTangencyId
      )
        continue;
  ...
  for (let i = 0; i < arcs.length; i++) {
    for (let j = i + 1; j < arcs.length; j++) {
      ...
      // Skip adjacent
      if (a1.endTangencyId === a2.startTangencyId || a1.startTangencyId === a2.endTangencyId)
        continue;
