...
export function ContactPathRenderer({
  path,
  visible,
  color = '#FF4500',
  width = 4,
}: ContactPathRendererProps) {
    ...
    <g className="contact-path-envelope">
      {path.map((seg, idx) => {
        if (seg.type === 'ARC') {
          ...
          const endX = seg.center.x + seg.radius * Math.cos(effEndAngle);
          const endY = seg.center.y + seg.radius * Math.sin(effEndAngle);
