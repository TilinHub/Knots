          ...
          onLoadEntry={(entry) => {
            ...
            if (entry.results.length > 0) {
              ...
              if (entry.diskSequence && knotState?.actions?.setSequence) {
                ...
                const chiralities =
                  (entry as any).chiralities ||
                  (entry.results?.[0] as any)?.combinatorial?.chiralities;
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>
              Each disk is positioned at the center of a closed region formed by the segments and
              arcs of the diagram.
            </p>
          </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div
                    style={{
                      ...
                      height: '10px',
                      borderRadius: '50%',
                      background: knot.color || '#FF8C00',
                    }}
                  ></div>
                  <span>
                    {knot.name || `Knot ${i + 1}`} ({knot.diskSequence.length} disks)
                  </span>
                </div>
