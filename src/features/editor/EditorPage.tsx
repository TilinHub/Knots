...
export function EditorPage({ onBackToGallery, initialKnot }: EditorPageProps) {
  ...
  const diskBlocks = React.useMemo(
    () => editorState.blocks.filter((b): b is CSDisk => b.kind === 'disk'),
    [editorState.blocks],
  );
  ...
  function reconstructFromFrozenPath(
    ...
    const recomputed: (any | null)[] = frozenPath.map((seg) => {
      ...
      const d1 = {
        id: startDiskId,
        center: d1Data.center,
        radius: d1Data.radius,
        regionId: '',
        color: '',
      };
      const d2 = {
        id: endDiskId,
        center: d2Data.center,
        radius: d2Data.radius,
        regionId: '',
        color: '',
      };
  ...
  const { savedKnotPaths, accumulatedObstacles } = useMemo(() => {
    ...
    editorState.savedKnots,
    contactDisksForGraph,
    diskBlocks,
    rollingState.isActive,
    rollingState.rollingDiskId,
    rollingState.getCurrentPosition,
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      <EditorHeader
        ...
        onToggleContactDisks={() =>
          editorActions.setShowContactDisks(!editorState.showContactDisks)
        }
        ...
        onToggleTransparentDisks={() =>
          editorActions.setTransparentDisks(!editorState.transparentDisks)
        }
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        <div style={{ flex: 1, minHeight: 0, minWidth: 0 }}>
          <CSCanvas
            ...
            // Dubins
            dubinsMode={dubinsState.state.isActive}
            dubinsPaths={(dubinsState.state.computedPaths || []).slice(
              0,
              dubinsState.state.maxPathsToShow,
            )}
          ...
          >
            ...
            {editorState.validation.valid ? (
              ...
              <ul style={{ color: 'var(--accent-error)' }}>
                {editorState.validation.errors.map((e: any, i: number) => (
                  <li key={i}>{e.message || JSON.stringify(e)}</li>
                ))}
              </ul>
            )}
            <button
              onClick={() => editorActions.setShowValidation(false)}
