...
export function useEditorState(initialKnot?: InitialKnot) {
  ...
  function addSavedKnot(
    ...
    if (frozenPath) {
      ...
      const findDiskForPoint = (pt: {
        x: number;
        y: number;
      }): { diskId: string; disk: { center: { x: number; y: number }; radius: number } } | null => {
        for (const [id, d] of diskLookup) {
          const dist = Math.sqrt(Math.pow(pt.x - d.center.x, 2) + Math.pow(pt.y - d.center.y, 2));
          if (Math.abs(dist - d.radius) < d.radius * 0.15) {
            // 15% tolerance
      ...
      enrichedPath = frozenPath.map((seg: any) => {
        if (seg.type === 'ARC') {
          ...
          if (sDisk) {
            enriched._startAngle = Math.atan2(
              seg.start.y - sDisk.center.y,
              seg.start.x - sDisk.center.x,
            );
  ...
  function addDisk() {
    ...
    if (diskBlocks.length > 0) {
      ...
      const rightMostDisk = diskBlocks.reduce((prev, current) => {
        return prev.center.x + prev.visualRadius > current.center.x + current.visualRadius
          ? prev
          : current;
      });
