...
5. For workflows that must survive failures, use durable execution at the infrastructure layer.

## Related Skills

Works well with: `event-sourcing-architect`, `saga-orchestration`, `workflow-automation`
...
Directly applicable to Knots' strict layered architecture (`core/` → `features/` → `renderer/`). Use when refactoring envelope computation strategies, adding new geometry modules, or enforcing the zero-UI-dependency rule in `core/`.
