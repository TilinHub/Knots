...
if (diagram) {
    ...
    log("--- Disks ---");
    diagram.disks.forEach(d => log(`D${d.index}: (${d.center.x.toFixed(4)}, ${d.center.y.toFixed(4)})`));
    ...
    diagram.segments.forEach(s => {
        ...
        log(`Seg ${s.startTangencyId}->${s.endTangencyId}: P(${pA.x.toFixed(2)},${pA.y.toFixed(2)})->P(${pB.x.toFixed(2)},${pB.y.toFixed(2)}) Vec(${vec.x.toFixed(2)},${vec.y.toFixed(2)})`);
